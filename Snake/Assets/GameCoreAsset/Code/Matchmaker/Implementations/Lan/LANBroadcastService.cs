...
public class LANBroadcastService : Singleton<LANBroadcastService>
{
    public int Port;

    public delegate void delJoinServer(string strIP,string data); // Definition of JoinServer Delegate, takes a string as argument that holds the ip of the server
    ...
    private struct ReceivedMessage { public float fTime; public string strIP; public bool bIsReady; public string additionalData; } // Definition of a Received Message struct. This is the form in which we will store messages
    ...
    void Update()
    {
        ...
        {
            var state = currentState == enuState.Announcing ? strServerReady : strServerNotReady;
            var model = new BroadcastModel() { serverState = state, additionalData = AdditionalData };
            // Determine out of our current state what the content of the message will be
            byte[] objByteMessageToSend = System.Text.Encoding.ASCII.GetBytes(JsonUtility.ToJson(model));
            // Send out the message
            objUDPClient.Send(objByteMessageToSend, objByteMessageToSend.Length, new IPEndPoint(IPAddress.Broadcast, Port));
            // Restart the timer
        ...
        if (currentState == enuState.Searching)
        {
            ...
            foreach (ReceivedMessage objMessage in lstReceivedMessages)
            {
                ...
                if (objMessage.bIsReady)
                {
                    ...
                    delWhenServerFound(objMessage.strIP, objMessage.additionalData);
                    break;
                }
            }
            ...
            if (currentState == enuState.Searching && Time.time > fTimeSearchStarted + fTimeToSearch)
            {
                ...
                string strIPOfServer = ipAddress;
    ...
    {
        return DateTime.Now.Ticks / 1e7f; 
    }
    ...
    private void StartBroadcastingSession()
    {
        ...
        // Create the client
        objUDPClient = new UdpClient(Port);
